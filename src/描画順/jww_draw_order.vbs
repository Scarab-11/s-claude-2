...
Function ReadGroup(lines, i, curLg, curLy, curLc, curLt, _
    ...
    Else
        ...
        ' 曲線属性(pl) には # を付けない。
        '
        ' 実機で確認した結果、# を付けても付けなくても、書き戻した
        ' 曲線属性は「線色ごとのまとまり」に分かれる。結果は同じ。
        ' Jw_cad は書き戻しを読むとき、線色の行が来たところで曲線属性の
        ' まとまりを閉じるためで、# の有無とは関係が無い。
        '
        ' 付けない側を採るのは、Jw_cad 自身が「次が属性行のときは # を
        ' 省く」形で書き出しており、その形に合わせるため。
        ' 出力側では、まとまりの直後に属性行を必ず置いて閉じる。
...
Sub WriteResult(tempPath)
    ...
    For i = 0 To gRecCount - 1
        ...
        ' 曲線属性(pl) のまとまりの後ろには属性行を必ず置く。次の
        ' レコードが線色の行を書き直すように、覚えている値を捨てておく。
