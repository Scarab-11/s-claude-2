...
| `Flux1_StyleBlend_ShapeLocked.json` | 上の版に **ControlNet 2 段（深度＋線）で形の固定**を足したもの。プロンプト無しでも勝手な被写体が湧かない。8GB VRAM 向け |
...
山の画像 2 枚を入れ、プロンプトを空にして実行すると、出力の山肌に人物の顔が現れた。

**空プロンプトは「何も指定しない」ではなく「モデルの事前分布に任せる」という指定。**
Flux dev はそこで人物の顔に落ちる。このワークフローには形を固定する経路が無い
（ControlNet を使わない構成なので）ため、被写体を押さえるものが他に無い。

顔を誘発する条件が重なっていた。

| 条件 | 旧既定値 | 影響 |
|---|---|---|
| プロンプト | 空にした | 被写体を決めるものが無い |
| 「画風A の強さ」「画風B の強さ」 | `0.6` | Redux トークンを 0.6 倍するので画像側の主張も弱い |
| 「width（生成幅）」「height（生成高さ）」 | `832 × 1216` | 肖像画の比率。縦長は顔を誘発する |
| 「denoise ◀ 元の2枚をどれだけ残すか」 | `0.60` | 重ねた画像の曖昧な陰影が描き直され、顔に解釈される |

既定値を次のように変えた。

| ノード | 旧 | 新 |
|---|---|---|
| 「プロンプト（被写体を1語 ＋ 画材）」 | 肖像画の文 | `a mountain landscape, oil painting on canvas, …` |
| 「画風A の強さ」「画風B の強さ」 | `0.6` | `0.9` |
| 「denoise ◀ 元の2枚をどれだけ残すか」 | `0.60` | `0.45` |
| 「width（生成幅）」「height（生成高さ）」 | `832 × 1216` | `1024 × 1024` |

**プロンプトの冒頭に被写体を 1 語だけ書く。** 残りは画材と質感。

| 題材 | 冒頭に書く語 |
|---|---|
| 風景 | `a mountain landscape` / `a seascape` / `a city street` |
| 人物 | `a portrait` |
| 静物 | `a still life` |
| 建物 | `an architectural view` |

「重ねた結果（出発点）」に写っているものと、この語を一致させる。食い違うと破綻する。

元画像の出どころにも注意する。顔用のワークフローで作った画像を入れると、
その中に既に顔の構造が入っていることがあり、Redux はそれを忠実に拾う。
...
| `0.45` | 既定値。形を引き継ぎつつ絵として整う |
| `0.60` | 描き変わる。被写体そのものが変わることがある |
| `0.80` | かなり描き変わる |
...
| 変更するノード | 変更内容 |
|---|---|
| 「denoise ◀ 元の2枚をどれだけ残すか」 | `0.45` → `0.30` |
| 「FluxGuidance ◀ 絵の磨き具合」 | `3.5` → `2.0`〜`2.5` |
| 「プロンプト（被写体を1語 ＋ 画材）」 | 被写体の後は画材と質感だけにする |

プロンプト例:

```
a mountain landscape, watercolor on rough paper, loose wet-on-wet washes,
visible paper texture, muted palette, unfinished edges, soft focus
```

被写体は 1 語で足りる。詳しく描写すると重ねた画像と喧嘩する。
...
| 症状 | 対処 |
|---|---|
| 片方の絵がそのまま出てしまう | その側の `strength` を 0.5 前後まで下げる |
| どちらの画風も乗らない | 両方 1.1 に上げる |
| 混ざらず継ぎ接ぎに見える | 2 枚の明度・彩度を近づけてから読み込む |
| 関係の無い顔や物が出る | 両方上げる。下げるほどモデルの事前分布が前に出る |

**下げすぎると画像側の主張が消える。** `0.6` 以下は顔が出やすくなるので、
プロンプトに被写体を書いた上で使う。
...
「width（生成幅）」「height（生成高さ）」は `1024 × 1024`。
**元画像と同じ比率に合わせる。** 横長の風景なら `1216 × 832`、
縦長の人物なら `832 × 1216`。比率がずれると参照画像が詰め込まれ、構図ごと作り直される。

縦長 `832 × 1216` は肖像画の比率で、それ自体が顔を誘発する。
風景を扱うときに縦長のままにしない。
...
上の版と同じ構成に、**重ねた画像から取った形を ControlNet で固定する経路**を足したもの。
`Flux1_ShapeOnly_FaceFromImage2.json` と同じく **1段目（深度）と 2段目（線）の直列**で、
2段目は Ctrl+B で切り替える。
...
形:   Image1 ┐
              ├ 2枚を重ねる ┬ latent 化 ───────────────┐
      Image2 ┘              ├ 前処理 1段目（深度）→ 1段目 ┤
                            └ 前処理 2段目（線）  → 2段目 ┤
                                                        ↓
画風: Image1 → 読み取る → 画風A の強さ ┐             サンプラー
      Image2 → 読み取る → 画風B の強さ ┴→ 1段目 → 2段目 ┘
```
...
| 段 | 渡すもの | 得意なこと | 既定 |
|---|---|---|---|
| 1段目 | 深度マップ | 立体・奥行き・構図 | 有効 |
| 2段目 | 線 | 輪郭・細部の位置 | **無効** |

**2段目は既定で無効。**
「形を固定する 2段目（Apply ControlNet）◀ Ctrl+B で切替」を選んで
**Ctrl+B** を押すと有効になる。もう一度押すと戻る。

深度だけで足りているなら 1段目のままでよい。輪郭がぼやける・細部が溶けるときに足す。

| 症状 | どうするか |
|---|---|
| 立体は合うが輪郭がぼやける | 2段目を有効にする |
| 建物や水平線が歪む | 2段目を有効にして `MLSDdetector` |
| 線が絵に残って硬い | 2段目の `end_percent` `0.6` → `0.4` |
| 線が効かない | 2段目の `strength` `0.45` → `0.70` |
| 線がざらつく | 2段目の `resolution` `1024` → `512` |

2段目に渡す線の種類は
「重ねた画像の前処理 2段目（AIO Aux Preprocessor）◀ 種類を選ぶ」で選ぶ。
既定は `HEDPreprocessor`。**重ねた画像は境界が曖昧なので、硬い線より柔らかい線が合う。**
人物・静物なら `LineArtPreprocessor`、建築なら `MLSDdetector`。

### 既定値

| ノード | 値 | 理由 |
|---|---|---|
| 「プロンプト（空でよい）」 | 空 | 形は ControlNet が押さえるので不要 |
| 「ネガティブプロンプト（空でよい）」 | 空 | 形を固定するノードが両方要求するので置いてある。2 段で共用 |
| 「形を固定する 1段目（Apply ControlNet）」 | `1.0` / `0.0` / `1.0` | `end_percent` が `1.0` であることが要点 |
| 「形を固定する 2段目（Apply ControlNet）◀ Ctrl+B で切替」 | `0.45` / `0.0` / `0.6` ／ 無効 | 線は序盤で効かせ、後半は画風に任せる |
| 「Union の制御タイプ（1段目）」 | `depth` | 前処理を変えたらここも合わせる |
| 「denoise ◀ 元の2枚をどれだけ残すか」 | `0.30` | 重ねた画素をできるだけ残す |
| 「画風A の強さ」「画風B の強さ」 | `0.9` | 下げるほど事前分布が前に出る |

### 効いているかの確認

「1段目に渡る情報」と「2段目に渡る情報」を見る。**灰色か白黒であれば正常。**
色が付いていたら前処理が効いていない。
どちらもサンプラーを回さなくても更新されるので、線の出方はここで見比べてから決める。

### 形の固定を弱めたいとき

固定すると絵の自由度は下がる。ゆるめるなら上から順に。

| 変更するノード | 変更内容 |
|---|---|
| 「形を固定する 1段目（Apply ControlNet）」 | `end_percent` `1.0` → `0.7` |
| 同上 | `strength` `1.0` → `0.7` |
| 「denoise ◀ 元の2枚をどれだけ残すか」 | `0.30` → `0.45` |

**1段目の `end_percent` を下げるほど後半が自由になり、顔が湧く余地が戻る。**
`1.0` のままなら湧かない。2段目の `end_percent` は下げてよい。形は 1段目が握っている。
...
ControlNet の重みは 2 段で共用するので、VRAM の増分は 4GB 程度のまま。
