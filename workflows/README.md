...
| 決めるもの | ノード | 値 |
|---|---|---|
| 顔の大きさ | 「Image2 の顔の大きさ」 | `width` / `height` |
| 顔の位置 | 「Image2 の位置を決める（仮置き）」 | `x`（右が正）/ `y`（下が正） |

1. まず「貼り付け結果（描き直す前）」を見る
2. Image2 の顔が大きすぎるなら `width` / `height` を小さくする
3. 位置がずれているなら `x` / `y` でずらす
4. 顔がマスクの位置にぴったり重なったら実行

**ここが合っていないと、いくら設定を変えてもきれいにならない。**
...
マスクを **source のサイズに合わせて拡縮する**ため。source を縮めた状態でマスクを
渡すと、マスクごと縮んで顔の位置と対応しなくなる。

### 出力が写真のままで Image1 の絵柄にならないとき

「denoise ◀ 顔をどれだけ描き直すか」を上げる。ここが一番効く。
マスクの外は一切変わらないので、上げても顔以外は劣化しない。

| `denoise` | 結果 |
|---|---|
| `0.40` | Image2 の写真の質感がそのまま残る。浮いて見える |
| `0.60` | まだ写真寄り |
| `0.75` | 既定値。Image1 の絵柄で描き直されつつ顔立ちは残る |
| `0.90` | ほぼ描き直し。Image2 の面影は薄くなる |

顔立ちを取るか絵柄を取るかの綱引きになる。0.75 から始めて調整する。
...
Image2 ─ 顔の大きさ → 位置を決める（仮置き） → 顔をマスクの形で合成
...
| 症状 | 変更するノード | 変更内容 |
|---|---|---|
| 写真の質感が残る・浮いて見える | 「denoise ◀ 顔をどれだけ描き直すか」 | `0.75` → `0.85` |
| 顔が Image2 に似ない | 同上 | `0.75` → `0.60` |
