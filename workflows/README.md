...
手順 3 に成功すると、ノードのファイル名表示が
`clipspace/clipspace-mask-....png [input]` に変わる。これがマスクが入った証拠。
...
| ノード | 全ゼロマスクでの挙動 |
|---|---|
| 「Image2 の顔を貼り付け」 | Image2 を 1 ピクセルも貼らない |
...
Image1 ─┬ 解像度を取得
  ...
  マスク ─ 少し広げる → 縁をぼかす → 上の 3 つと「マスク内だけ描き直す」へ

Image2 ─ 解像度を合わせる → 顔を貼り付け

顔を貼り付け → latent 化 → マスク内だけ描き直す → サンプラー
サンプラー → latent → 画像 → マスク外を Image1 に戻す → 最終出力
```

1. LoadImage の `MASK` 出力は `1. - alpha`（`nodes.py:1787`）なので、
   MaskEditor で塗った領域が `1` になる。
2. 「Image2 の顔を貼り付け」がその領域に Image2 を貼る（雑なコラージュ）。
...
「Image2 の顔を貼り付け」の `x` / `y` で Image2 をずらせる。
「貼り付け結果（描き直す前）」を見ながら、顔がマスクの位置に来るよう調整する。
Image1 と Image2 で顔の大きさや向きが大きく違うと破綻するので、
**似た構図の 2 枚**を使うのが前提。
...
| 症状 | 変更するノード | 変更内容 |
|---|---|---|
| 顔が Image2 に似ない | 「denoise ◀ 顔をどれだけ描き直すか」 | `0.55` → `0.40` |
| 貼り付けた跡が浮いて見える | 同上 | `0.55` → `0.70` |
| 継ぎ目の線が見える | 「マスクの縁をぼかす」 | 4 つの値を `24` → `48` |
| 顔だけ画風が違う | 「画風の適用（Redux）」 | `strength` `0.8` → `1.0` |
