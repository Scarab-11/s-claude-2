...
echo.
echo ---- exe を作ります  %MODE% ----
"%PY%" -m PyInstaller %MODE% --console --clean --noconfirm --noupx --name shiage --workpath "%~dp0_build" --distpath "%~dp0_dist" --specpath "%~dp0_build" "%~dp0shiage.py"
if errorlevel 1 (
  echo ビルドに失敗しました。
  pause
  exit /b 1
...
mkdir "%OUT%"
if /i "%MODE%"=="--onefile" (
  copy /y "%~dp0_dist\shiage.exe" "%OUT%\" >nul
) else (
  xcopy /e /i /y "%~dp0_dist\shiage" "%OUT%" >nul
)
copy /y "%~dp0仕上表_縦組み.bat" "%OUT%\" >nul
copy /y "%~dp0shiage_setting.txt" "%OUT%\" >nul
...
echo.
echo  設定を変えるときは、同じフォルダの shiage_setting.txt を
